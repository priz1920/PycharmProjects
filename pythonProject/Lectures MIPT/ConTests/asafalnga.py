...
N = int(f.readline())  # 0 <= N <= 100 - количество дней
fence = []
day = [0, 0, 0]
for k in range(N):
    for i in range(3):
        day[i] = int(f.readline())
    if day[0] > day[1]:
        continue
    print("день", k, day)
...
M = int(f.readline())  # 0 < M <= 100 - число интересующих Васю дощечек
search = []
for j in range(M):
    plank = int(f.readline())  # Номера интересующих дощечек
    search.append(plank)
result = []
